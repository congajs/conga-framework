		...
		if (parts.length > 1){
			...
			for (var i=0, j=this.paths.length; i<j; i++){
				...
				try {
					...
					console.log('try: ' + path.join(namespacePath, parts[0], 'lib', parts[1]));

					return require(path.join(namespacePath, parts[0], 'lib', parts[1]) + '.js');
				} catch(err){
					//console.log(err);
