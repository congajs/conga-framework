...
module.exports = class RouteRunner {
    ...
    run(container, route, request, response) {
        ...
        container.get('logger').debug('[conga-framework] - request: ' + request.method + ' ' + route.path);
        ...
        this.initializeRequestScope(container, controller, request, response, route, (scope) => {
    ...
    runControllerInScope(scope, cb) {
        try {
            if (typeof scope.controller[scope.route.action] === 'function') {
                ...
                if (typeof scope.route.injectedServices !== 'undefined' &&
