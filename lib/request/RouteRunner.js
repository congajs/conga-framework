...
module.exports = class RouteRunner {
    ...
    _copyServiceIntoRequestScope(globalContainer, scopedContainer, sid) {
        ...
        // Services added manually won't have a definition
        if (!definition) {
            const service = Object.create(_proto);
        ...
        if (definition.isFunction()) {
            const serviceLoader = new ServiceLoader(globalContainer.__services['bundle.finder']);
            const func = serviceLoader.load(definition.getFunction());
            if (!(func instanceof Function)) {
                return null;
            }
            const service = func(...args);
            scopedContainer.__services[sid] = service;
            return service;
        }
