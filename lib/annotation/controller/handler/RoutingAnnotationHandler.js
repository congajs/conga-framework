...
// third-party modules
const annotations = require('@conga/conga-annotations');
...
module.exports = class RoutingAnnotationHandler {
	...
	handleAnnotations(container, reader, controller) {
		...
		// 	// store routes for express to use later on
		// 	container.setParameter(
		// 		'conga.websocket.routes',
		// 		container.getParameter('conga.websocket.routes').concat(routes.websocketRoutes)
		// 	);
		// }
	}
	...
	parseRoutesFromFile(container, reader, controller) {
		...
		const C = require(controller.filePath);
