							...
							function(){
								...
								container.get('conga.filter.runner').run(
									...
									function(){
										...
										// clone the response and request for our controller
										var _res = clone(res, false);
										var _req = clone(req, false);

										try {
											if (typeof controller[route.action] === 'function') {

												// call the controller method
												controller[route.action].call(controller, _req, _res);

											} else {
												container.get('logger').error(route.controller + '.' + route.action + ' is not a function');
												_res.return({error: 'Internal Server Error'}, 500);
											}
										} catch (err){
