...
ExpressListener.prototype = {
	...
	enhanceRequest: function(req, res, next){
		...
		// convenience method to check if this is a jsony request
		req.isJson = function(){
	...
	onKernelServerBoot: function(event, next){
		...
		try {
			...
			container.get('logger').debug('server started at http://localhost:' + container.get('config').get('framework').app.port);
