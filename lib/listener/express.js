...
ExpressListener.prototype = {
    ...
    createRequestScope: function(controller, container, req, res){
        ...
        var _get = container.get;
        _container.get = function(id){
            var service;
            var scope = _container.getScope(id);
            if (scope == 'request') {
                if (typeof _container.__services[id] === 'undefined') {

                    // Initialize a copy of the service
                    var copyService = function(_id){
                        var svc;
                        var _proto = _get.call(container, _id);
                        if (_proto) {
                            svc = Object.create(_proto);

                            // Parse constructor arguments and apply
                            var definition = container.__definitions[_id];
                            var args = [];
                            if (definition.hasArguments()) {
                                ...
                                for(var i=0, j=defArgs.length; i<j; i++){
                                    ...
                                    if(typeof a == 'string' && a[0] == '@'){
                                        var sid = a.replace('@', '');
                                        if (!_container.__services[sid]){
                                            _container.__services[sid] = copyService(sid);
                                            if (_container.getScope(sid) != scope){
                                                throw new Error("Service scope mismatch error.");
                                            }
                                        }
                                        args.push(_container.__services[sid]);
                                    } else {
                                        args.push(a);
                                    }
                                }
                            ...
                            // Call constructor
                            svc.constructor.apply(svc, args);
                                ...
                                for(var i = 0, j = calls.length; i < j; i++){
                                    ...
                                    for(var x=0, y=calls[i].arguments.length; x < y; x++){
                                        if(typeof calls[i].arguments[x] == 'string' && calls[i].arguments[x][0] == '@'){
                                            var sid = calls[i].arguments[x].replace('@', '');
                                            if (!_container.__services[sid]){
                                                _container.__services[sid] = copyService(sid);
                                                if (_container.getScope(sid) != scope){
                                                    throw new Error('Service scope mismatch error.');
                                            ...
                                            callArgs.push(_container.__services[sid]);
                                        } else {
                                            callArgs.push(calls[i].arguments[x]);
                                        }
                                    }
                                    // Apply method
                                    svc[method].apply(svc, callArgs);
                                }
                            }
                    ...
                    try {
                        service = copyService(id);
                        _container.__services[id] = service
                    } catch(e){
                        container.get('logger').error(e.message);
                        service = null;
                    }

                } else {
                    service = _container.__services[id];
                }
            } else {
                service = _get.call(container, id);
            }
			...
			(function(container){
				...
				app[verb](route.path, function(req, res){
					...
					var run = function(){
                        ...
                        if (typeof config.request_scope !== 'undefined'){
                            scopeEnabled = config.request_scope.enabled;
                        }
