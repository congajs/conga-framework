...
module.exports = class ControllerAnnotationListener {
    ...
    onKernelCompile(event, next) {
        ...
        console.log(event.container.getParameter('kernel.context'));
        process.exit();

        if (!event.container.get('config').get('framework').controller.contexts.includes(
            event.container.getParameter('kernel.context')
        )) {
            return next();
        }
