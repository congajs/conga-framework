    ...
    mergeDefaults() {
        ...
        for (const bundle in bundlePaths) {
            ...
            if (fs.existsSync(defaultConfigPath)) {
                ...
                let baseBundleConfigHandlerPath = path.join(
                    bundlePaths[bundle],
                    'lib',
                    'dependency-injection'
                );

                if (!fs.existsSync(baseBundleConfigHandlerPath)) {
                    baseBundleConfigHandlerPath = path.join(
                        bundlePaths[bundle],
                        'lib',
                        'configuration'
                    );
                    if (!fs.existsSync(baseBundleConfigHandlerPath)) {
                        throw new Error('Could not find dependency-injection configuration directory.');
                    }
                }

                // try Configuration.js and configuration.js for legacy purposes
                let bundleConfigHandlerPath = path.join(baseBundleConfigHandlerPath, 'Configuration.js');

                if (!fs.existsSync(bundleConfigHandlerPath)) {
                    bundleConfigHandlerPath = path.join(baseBundleConfigHandlerPath, 'configuration.js');
                }

                let configName = require(bundleConfigHandlerPath).getName();
                let defaultConfig = this.readConfigFile(defaultConfigPath);
