...
module.exports = class Loader {
	...
	findBundlePaths() {
		...
		bundles.forEach((bundle) => {
			...
			if (typeof this.config.parameters['bundle.paths'][bundle] !== 'undefined') {
				return;
			}
	...
	loadAppConfig() {
		...
		let configFile = path.join(this.projectPath, this.app, 'config', 'config_' + this.environment + '.yml');
	...
	mergeDefaults() {
		...
		for (var bundle in bundlePaths) {
			...
			if (fs.existsSync(defaultConfigPath)) {
				...
				let bundleConfigHandlerPath = path.join(bundlePaths[bundle], 'lib', 'dependency-injection', 'configuration');
